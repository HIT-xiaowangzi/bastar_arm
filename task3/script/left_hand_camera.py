...
#模板匹配确定背带牛的位置及姿态角
def rotate_template(template,degree):
    temp=template.copy()
    rotate_matrix=cv.getRotationMatrix2D((temp.shape[0]/2,temp.shape[1]/2),degree,1)
    rotate_temp=cv.warpAffine(temp,rotate_matrix,(temp.shape[0],temp.shape[1]))
    return rotate_temp
def match_trape(template,src):
    s = []
    for i in range(-18,18,1):
        rotate_temp= rotate_template(template,10*i)
        match_result = cv.matchTemplate(src, rotate_temp, cv.TM_SQDIFF_NORMED)#TM_SQDIFF:平方差匹配 值越大，匹配效果越差
        minval,maxval,minloc,maxloc=cv.minMaxLoc(match_result)
        s.append([i,minval,minloc[0],minloc[1]])
    ...
    for j in range(len(valid)):
        dist=(valid[j][2]+rotate_temp.shape[0]/2-340)^2+(valid[j][3]+rotate_temp.shape[1]/2-190)^2
        length.append(dist)
    res=np.argmin(length)
    x_center=valid[res][2]+rotate_temp.shape[0]/2
    y_center=valid[res][3]+rotate_temp.shape[1]/2
    theta=valid[res][0]*10
...
def near(img):
    ...
    # colorLow = np.array([10, 10, 30])
    # colorHigh = np.array([80, 50, 127])
    #     #colorLow1 = np.array([0, 80, 46])
    #     #colorHigh1 = np.array([10, 255, 255])
    # mask_test = cv.inRange(hsv, colorLow, colorHigh)
    copyimg = cv.cvtColor(img, cv.COLOR_RGB2GRAY)
    _, mask = cv.threshold(copyimg, 125, 255, cv.THRESH_BINARY)
   # _, mask = cv.threshold(mask_test, 125, 255, cv.THRESH_BINARY)
    element = cv.getStructuringElement(cv.MORPH_RECT, (3, 3))
    ...
    area_list=[]
    #拍摄深度图
    template=cv.imread("/home/wangzirui/HIT-DLR/src/task2/files/depth_mask.jpg",cv.IMREAD_GRAYSCALE)
    ...
    draw2 = draw1.copy()
    x_center,y_center,theta=match_trape(template,draw2)
    cv.circle(draw2,(x_center,y_center),5,0,2)
    return x_center,y_center,theta,draw0,draw2
...
def callback0(imgmsg):
    ...
    x_center,y_center,theta,draw0,draw2=near(image)
