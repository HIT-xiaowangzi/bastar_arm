...
geometry_msgs::Pose Callback03(uint16_t x,uint16_t y)
{
    ...
    if(isnan(camera_z)||camera_z==0)
    {
        ...
        for(i=-3;i<=3;i++)
        {
            for(j=-3;j<=3;j++)
            { 
                if(!isnan(depth_frame.at<uint16_t>(y+i,x+j)))
                {
                    temp+=(double)depth_frame.at<uint16_t>(y+i,x+j)/1000;
                    count++;
                }
            }
        }
...
bool PointTransform(task2::GetPoint::Request &req,task2::GetPoint::Response &res)
{
    ...
    {
        //ROS_INFO("Enter PointTransform!");
        //std::cout<<req.result.MyLists.at(0).x<<std::endl;
        //std::cout<<req.result.MyLists.at(0).y<<std::endl;
        if(size<3)
        {
            // ROS_INFO("size:[%d]",size);
...
int main(int argc, char **argv)
{
    ...
    ros::ServiceServer service = n.advertiseService("task2_service", PointTransform);
    // 循环等待回调函数
    ROS_INFO("task2_Server is ready!");
