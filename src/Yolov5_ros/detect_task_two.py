...
def detect(save_img=False):
    ...
    pub_p = rospy.Publisher('beidainiu', MyListArray, queue_size=100)
    pub_box = rospy.Publisher('box_info', MyListArray, queue_size=100)
    pub_head=rospy.Publisher('head',MyListArray,queue_size=100)
    pub_width=rospy.Publisher('width',Int16,queue_size=100)
    rate = rospy.Rate(50)  # 10hz
    ListArray_cattle=MyListArray()
    ans_cattle=MyList()
    ListArray_box=MyListArray()
    ans_box=MyList()
    ListArray_head=MyListArray()
    ans_head=MyList()
    width=0
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    c=int(cls)
                    if c==0:#牛身子
                        if(xywh[0]<0.6):#牛在右半平面
                            ans_cattle.x=int(xywh[0]*640)
                            ans_cattle.y=int(xywh[1]*480)
                            ans_cattle.class_=c+2
                            ans_cattle.confidence=conf
                            ListArray_cattle.MyLists.append(deepcopy(ans_cattle))
                    elif c==1:#牛头
                        ans_head.x=int(xywh[0]*640)
                        ans_head.y=int(xywh[1]*480)
                        ans_head.class_=c+2
                        ans_head.confidence=conf
                        ListArray_head.MyLists.append(deepcopy(ans_head))
                    else :#箱子
                        ans_box.x=int(xywh[0]*640)
                        ans_box.y=int(xywh[1]*480+xywh[3]*480*0.25)
                        ans_box.class_=c+2
                        ans_box.confidence=conf
                        ListArray_box.MyLists.append(deepcopy(ans_box))#箱子
                        # ans_box.x=int(xywh[0]*640)
                        # ans_box.y=int(xywh[1]*480-xywh[3]*480*0.25)
                        # ans_box.class_=c+2
                        # ans_box.confidence=conf
                        # ListArray_box.MyLists.append(deepcopy(ans_box))#箱子
                        width=int(xywh[2]*640)
                    #获取距离信息
                    distance_list = []  
            ...
            pub_p.publish(ListArray_cattle)
            #rate.sleep()
            ListArray_cattle.MyLists.clear()
            pub_head.publish(ListArray_head)
            ListArray_head.MyLists.clear()
            pub_box.publish(ListArray_box)
            ListArray_box.MyLists.clear()
            pub_width.publish(deepcopy(width))
            width=0
            rate.sleep()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/wangzirui/yolov5/runs/train/exp16/weights/best.pt', help='model.pt path(s)')
