...
import rospy
from task1.msg import MyList,MyListArray
...
def detect(save_img=False):
    ...
    pub_p = rospy.Publisher('xyandclass', MyListArray, queue_size=100)
    pub_box = rospy.Publisher('box_info',MyListArray, queue_size=100)
    rate = rospy.Rate(50)  # 10hz
    ListArray_block=MyListArray()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    c=int(cls)
                    if c==0:
                        if(xywh[2]>0.3  or xywh[3]>0.3):
                            continue
                        else:
                            ans_block.x=int(xywh[0]*640)
                            ans_block.y=int(xywh[1]*480)
                            ans_block.class_=c+1
                            ans_block.confidence=conf
                            ListArray_block.MyLists.append(deepcopy(ans_block))#积木块
                    else:
            ...
            pub_p.publish(ListArray_block)
            #rate.sleep()
            ListArray_block.MyLists.clear()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/wangzirui/yolov5/runs/train/exp14/weights/best.pt', help='model.pt path(s)')
